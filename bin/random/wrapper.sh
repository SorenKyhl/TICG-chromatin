...
do
  ...
  bash ~/TICG-chromatin/bin/random/random${i}.sh $chiMethod $k $m $dataFolder $startSampleI $relabel $tasks $samplesPerNode $samplesPerTask $diagMethod $scratchDir $i $nSweeps $lmbda $minChi $maxChi $fillDiag $chiSeed $maxDiagChi
done
