...
sourceFile=~/TICG-chromatin/bin/datasets/diag_dataset88_imr90/diag_dataset_fns.sh
start=1
end=1000
for i in {6..10}
do
  echo $i $start $end
  # sbatch ~/TICG-chromatin/bin/datasets/bash_files/diag_dataset${i}.sh $sourceFile $start $end 128 "88_"
  start=$(( $start + 1000 ))
  end=$(( $end + 1000 ))
done
