...
# python  ~/TICG-chromatin/bin/datasets/generate_params.py --samples 10000 --b 180 --phi 0.008 --k 10 --ar 1.5 --m 512 --dataset $dataset --seq_mode 'eig_norm' --diag_mode 'meanDist_S_grid' --data_dir '/project2/depablo/erschultz'
#
...
do
  echo $i $start $end
  sbatch ~/TICG-chromatin/bin/datasets/diag_dataset${i}.sh $sourceFile $start $end 128 "58_"
  start=$(( $start + 2000 ))
  end=$(( $end + 2000 ))
done
