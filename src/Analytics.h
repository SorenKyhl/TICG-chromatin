...
public:
	...
	void lapTimer()
	{
		...
		totaltime = std::chrono::duration_cast<std::chrono::seconds>(stop-start);
		blocktime = std::chrono::duration_cast<std::chrono::seconds>(stop-laststop);
		laststop = stop;
	}
	...
	void log(int sweep)
	{
		...
		{
			std::cout << "Rate (this block)" << beads_moved_this_block/blocktime.count() << " beads/s \n";
		}
	}
};
