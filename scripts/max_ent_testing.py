...
from contact_map import getArgs, plot_all
...
def fit(dataset, sample, samples='samples', bl=140, phi=0.03, v=None, vb=None,
    ...
    root = osp.join(dir, f'{root}-max_ent{k}_test10')
    if osp.exists(root):
        shutil.rmtree(root)
    ...
    params['production_sweeps'] = 10000
    ...
    with open(osp.join(root, 'log.log'), 'w') as sys.stdout:
        me = Maxent(root, params, config, seqs, y, fast_analysis=True,
                    final_it_sweeps=0, mkdir=False, bound_diag_chis=False)
        t = me.fit()
