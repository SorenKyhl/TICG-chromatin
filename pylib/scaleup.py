...
def tune_stiffness(nbeads_large, nbeads_small, pool_fn, grid_bond_ratio, method):
    ...
    k_angle_opt =  optimize_stiffness(small_config, hic_pooled, low_bound=0, high_bound=2, method=method)
    return k_angle_opt
...
def scaleup(nbeads_large, nbeads_small, pool_fn, method="bayes"):
    ...
    # tune grid size
    try:
        k_angle_opt = tune_stiffness(nbeads_large, nbeads_small, pool_fn, grid_bond_ratio, method)
    except FileExistsError:
